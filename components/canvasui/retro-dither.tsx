...
export function RetroDither({
  ...
  useEffect(() => {
    ...
    // WebGL init can fail even when html-in-canvas is advertised — remount
    // children on the non-canvas fallback path (createRetroDither returned null).
    // eslint-disable-next-line react-hooks/set-state-in-effect -- recover from GPU init failure
  ...
  return (
    <div className={className} style={{ position: "relative", ...style }}>
      {/* Both canvases are position:absolute in native mode, so the wrapper
          would collapse to 0 height without in-flow size. The docs page sets
          an explicit full-viewport height; block embeds need this sizer. */}
      {native ? (
