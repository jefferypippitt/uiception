...
function BrandIcon({
  ...
  return (
    <span className={cn("relative flex shrink-0", className)}>
      ...
      <DarkIcon className="absolute inset-0 h-full w-full hidden dark:block" aria-hidden />
    </span>
  )
}
...
export default function DownloadsPanel({
  ...
  // Motion-blur style: blur + upward translateY proportional to remaining velocity
  ...
  return (
    ...
    >
          ...
          <div ref={anchorRef} className="w-fit">
            <ComboboxTrigger
              className={cn(
                ...
                "shadow-xs outline-none transition-colors hover:bg-muted/50",
                "focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
              )}
            >
              <BrandIcon id={seriesId} className="size-4" />
              <ComboboxValue>
                {(val) => BRANDS.find((b) => b.id === val)?.label ?? "Pick package"}
              </ComboboxValue>
            </ComboboxTrigger>
          </div>
      ...
      <div className="col-span-full grid divide-border/80 border-t border-border/80 sm:grid-cols-3 sm:divide-x">
        <StatCell value={formatInt(weekly.value)} label="Weekly npm downloads" valueStyle={motion(weekly.blur)} />
        <StatCell value={formatInt(stars.value)} label="GitHub stars" valueStyle={motion(stars.blur)} />
        <StatCell value={formatInt(contributors.value)} label="Contributors" valueStyle={motion(contributors.blur)} />
      </div>
    </div>
  )
}
