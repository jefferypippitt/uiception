...
export function HeroV4Root({ children }: { children?: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      gsap.set(root.querySelectorAll("[data-hero-v4-reveal], [data-hero-v4-word]"), {
        clearProps: "all",
      })
      return
    }
    ...
    const ctx = gsap.context(() => {
      ...
      // reveals[0] = paragraph
      // reveals[1] = CTA buttons

      const tl = gsap.timeline()

      // 1. Title — blur swipe left to right (reading direction)
      tl.fromTo(
        words,
      ...
      // 2. Paragraph — fades up once headline is mostly done
      tl.fromTo(
        reveals[0],
        { opacity: 0, y: 16 },
      ...
      // 3. CTAs — slight scale pop, feels intentional as the action layer
      tl.fromTo(
        reveals[1],
        { opacity: 0, y: 10, scale: 0.96 },
        { opacity: 1, y: 0, scale: 1, duration: 0.48, ease: "power2.out", clearProps: "transform" },
        "-=0.28"
      )
    }, root)
