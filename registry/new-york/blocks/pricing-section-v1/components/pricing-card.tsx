    ...
    <li className="min-w-0">
      ...
      >
        <CardHeader className="gap-3 border-b px-5 pt-5 pb-4">
          {plan.popular ? (
            <span className="absolute right-4 top-4 rounded-full border border-border bg-muted px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground">
              Most popular
            </span>
          ) : null}
          ...
          <div className="flex items-end gap-1">
            ...
            {plan.priceSuffix ? (
              <p className="pb-1 text-sm text-muted-foreground">{plan.priceSuffix}</p>
            ) : null}
          </div>
          <p className="text-sm text-muted-foreground">{plan.monthlyLabel}</p>
        </CardHeader>
        <CardContent className="flex-1 px-5 pt-4 pb-5">
          <p className="mb-3 text-sm font-medium text-foreground">{plan.intro}</p>
        ...
        <CardFooter className="border-t-0 bg-transparent p-4">
          <Button className="w-full rounded-full" variant={plan.ctaVariant ?? "default"}>
            {plan.ctaLabel}
          </Button>
        </CardFooter>
      </Card>
