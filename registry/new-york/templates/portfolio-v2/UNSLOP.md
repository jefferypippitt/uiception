...
bio in `app/page.tsx`, the project descriptions, and every note body in
`lib/notes.tsx`. This applies whenever `AGENTS.md` has you drafting prose
for this site, not just once.
...
17. **Padding past the point.** AI drafts tend to run long — an extra
    ...
    bio and each note, measured against what you're replacing — if you're
    over it, the fix is almost always to cut a sentence, not to tighten
    the wording of all of them.
18. **One idea per note, not three angles on it.** If two paragraphs are
